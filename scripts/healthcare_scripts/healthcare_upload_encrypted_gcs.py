...
GCS_OBJECT_PREFIX = "project1/healthcare_data"
GCP_CONN_ID = "gcp_gcs_conn"

# Sensitive columns to hash via encryption service
# (需要通过加密服务进行哈希的敏感字段)
# patient_uuid: primary key that links to all other systems (主键，可关联完整患者画像)
# medicare_number: Australian government-issued sensitive identifier (澳洲政府级敏感医保号)
TARGET_COLUMNS = ["patient_uuid", "medicare_number"]


def encrypt_and_upload_to_gcs() -> int:
    """
    Reads CSV files from healthcare_staging, hashes the patient_uuid and
    medicare_number columns via the shared encryption service, and uploads
    hashed files to GCS.
    (从 healthcare_staging 读取 CSV 文件，通过共享加密服务对 patient_uuid
    和 medicare_number 列进行哈希，然后上传到 GCS)
    ...
    try:
        ...
        if not valid_files:
            logger.info("No files found in healthcare_staging, nothing to upload (healthcare_staging 为空，无需上传)")
            return 0
        ...
        for file_name in valid_files:
            ...
            try:
                ...
                # Step 2: Hash each target column via encryption service
                # (第二步：依次对每个目标字段通过加密服务进行哈希处理)
                for col in TARGET_COLUMNS:
                    if col not in df.columns:
                        logger.warning(f"Column '{col}' not found in file, skipping (字段 '{col}' 不存在，已跳过): {file_name}")
                        continue

                    raw_data = df[col].astype(str).tolist()
                    logger.info(f"Sending {len(raw_data)} records for column '{col}' to encryption service (发送 '{col}' 列 {len(raw_data)} 条记录至加密服务)")
                    ...
                    if response.status_code == 200:
                        df[col] = response.json().get("result")
                        logger.info(f"Hashing successful for column '{col}', replaced in memory ('{col}' 列哈希成功，已在内存中替换)")
                    else:
                        logger.error(f"Encryption service returned error for column '{col}' (加密服务对 '{col}' 列返回错误): {response.status_code}")
                ...
                # Re-raise to stop the entire task (重新抛出，终止整个任务)
                raise
