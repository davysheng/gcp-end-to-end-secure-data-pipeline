...
def transfer_landing_to_staging() -> int:
    """
    Moves all valid CSV files from healthcare_landing to healthcare_staging.
    (将 healthcare_landing 中的所有有效 CSV 文件移动到 healthcare_staging)
    ...
    try:
        ...
        if not valid_files:
            logger.info("No files found in healthcare_landing, nothing to transfer (healthcare_landing 为空，无需转移)")
            return 0
        ...
        for file_name in valid_files:
            ...
            try:
                ...
                logger.info(f"Moved (已移动): {file_name} → healthcare_staging/")
                moved_count += 1
