...
ARCHIVE_DIR = os.environ.get(
    "BANK_ARCHIVE_DIR",
    "./data/1_archive/bank_archive"
)


def archive_original_data() -> int:
    """
    Moves original unencrypted CSV files from bank_staging to bank_archive.
    (将 bank_staging 中的原始未加密 CSV 文件移动到 bank_archive)
    ...
    try:
        ...
        if not valid_files:
            logger.info("No files found in bank_staging, nothing to archive (bank_staging 为空，无需归档)")
            return 0
        ...
        for file_name in valid_files:
            ...
            try:
                ...
                logger.info(f"Archived (已归档): {file_name} → bank_archive/")
                moved_count += 1
