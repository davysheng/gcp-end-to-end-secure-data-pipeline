...
GCS_OBJECT_PREFIX = "project1/game_data"
GCP_CONN_ID = "gcp_gcs_conn"


def encrypt_and_upload_to_gcs() -> int:
    """
    Reads CSV files from game_staging, hashes sensitive columns via
    the shared encryption service, and uploads to GCS.
    (从 game_staging 读取 CSV 文件，通过共享加密服务对敏感列进行哈希处理，然后上传到 GCS)
    ...
    try:
        ...
        # Get valid CSV files only (只获取有效的 CSV 文件)
        valid_files = [
        ...
        if not valid_files:
            logger.info("No files found in game_staging, nothing to upload (game_staging 为空，无需上传)")
            return 0
        ...
        for file_name in valid_files:
            ...
            try:
                ...
                # Step 2: Hash match_id column via encryption service
                # (第二步：通过加密服务对 match_id 列进行哈希处理)
                if "match_id" in df.columns:
                    raw_data = df["match_id"].astype(str).tolist()
                    ...
                    if response.status_code == 200:
                        df["match_id"] = response.json().get("result")
                    ...
                    logger.warning("'match_id' column not found, uploading as-is ('match_id' 列不存在，原样上传)")
                ...
                logger.error(f"Cannot reach encryption service at {ENCRYPT_API_URL} (无法连接加密服务)")
                raise
